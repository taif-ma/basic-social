...
def all_messages(request):
    ...
    return render(request, "chat/all-messages.html", {'friends': friends})
...
def messages_with_one_friend(request, friend):
    if request.user.username == friend:
        return redirect(reverse_lazy('chat:all-messages'))
    try:
    ...
    friends_two = Friend.objects.filter(user=request.user, status='friend')
    friends = friends_one | friends_two
    return render(request, "chat/friend-messages.html", {
        'friends': friends,
